...
Future<void> main() async {
  ...
  final composed = AsyncRange.generate(10, (i) => Future.value(i))
      .where((x) => x.isEven)
      .map((x) => x * 3)
      .take(4);
  ...
  // -----------------------------------------------------------------------
  print('\nexpand([x, x*10]):');
  final expanded = AsyncRange.fromIterable([1, 2, 3])
      .expand((x) => [x, x * 10]);
  ...
  // -----------------------------------------------------------------------
  print('\ntakeWhile(< 5):');
  final tw = AsyncRange.fromIterable([1, 2, 3, 4, 5, 6, 7])
      .takeWhile((x) => x < 5);
  print(await tw.toList()); // [1, 2, 3, 4]

  print('\ndropWhile(<= 3):');
  final dw = AsyncRange.fromIterable([1, 2, 3, 4, 5])
      .dropWhile((x) => x <= 3);
  ...
  print('\ndistinct:');
  final dist =
      AsyncRange.fromIterable([1, 1, 2, 2, 2, 3, 1, 1]).distinct();
  ...
  // -----------------------------------------------------------------------
  print('\nfollowedBy:');
  final cat = AsyncRange.fromIterable([1, 2, 3])
  ...
  // fold / reduce
  // -----------------------------------------------------------------------
  final sum = await AsyncRange.fromIterable([1, 2, 3, 4, 5])
      .fold<int>(0, (acc, x) => acc + x);
  print('\nfold sum: $sum'); // 15

  final product = await AsyncRange.fromIterable([1, 2, 3, 4, 5])
      .reduce((a, b) => a * b);
  ...
  final asyncMapped = AsyncRange.fromIterable([1, 2, 3]).asyncMap(
    (x) async {
